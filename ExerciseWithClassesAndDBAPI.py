...
class ListOfRecord:
    ...
    def read_sql(tableName, dbName):
        ...
        Important note:
        Prior to use this method, the table temperatures must be created:
        ...
        try:
            ...
            # an object (a ResultSet) is returned by the select statement.
            # data=cursor.fetchall() returns all rows of the ResultSet into a list of list
            
            while True:
                row = cursor.fetchone() # return a single row out of the ResultSet
                if row == None:
                    break
